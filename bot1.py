...
oder.add(putb, like, sub, watch, comment)
...
def keyboard(c):
	try:
		...
		if c.data == 'put':
			...
			bot.send_message(c.message.chat.id, '1️⃣ Нажмите кнопку оплатить и переведите нужную сумму \n 2⃣ Пришлите сюда скриншот чека с подписью в виде числа какую сумму вы пурувели \n 3⃣ После проверки мы пришлем вам сообщение ', reply_markup = oplata)
...
def messages(message):
	try:
		...
		if message.text == '💹Сделать заказ💼':
			...
			bot.send_message(message.chat.id, f'Что вы хотите накрутить? \n Цены указаны за один лайк/коммент и тд. \n Сначала пополните баланс до необходимой суммы. \n\n Ваш баланс: {db.getBalance(message.from_user.id)}', reply_markup=oder)
...
⛔️ Рекламодателям запрещено:
• Удалять бота из администраторов ранее 7 дней с момента заказа.
• Лишать бота прав ранее 7 дней с момента заказа.
• Менять логин канала  ранее 7 дней с момента заказа.
• Размещать к раскрутке материалы содержащие (зоо, педо, ПАВы, терроризм, шок контент и тд...).
• Удалять или изменять пост до завершения заказа.
• Создавать заказы причиняющиее какой либо вред, пользователю или его устройству.
• Создавать заказы, которые заведомо невозможно выполнить.
• Писать не ясные или размытые описания к заданям.
• Добавлять к раскрутке конкурирующие сервисы

• Рекламодателю может быть отказано в размещении заказа - без объяснения причин!
